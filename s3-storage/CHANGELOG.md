...
### Changed
- **BREAKING:** the serde `Config` struct and the free `new()` function are replaced by `S3Storage::builder(bucket)`, with the multipart defaults owned privately by the builder; config-file schemas belong to the server crates. The `serde` feature now gates only `Serialize`/`Deserialize` impls on the `PartSize` and `MultipartThreshold` newtypes: `PartSize` makes values outside the S3 part bounds (5 MiB to 5 GiB) unrepresentable, `MultipartThreshold` caps the threshold at the S3 5 GiB `PutObject` limit, and a multipart threshold below the part size is rejected at build (previously an unenforced doc claim). The construction errors are the dedicated `EmptyBucket` and `ThresholdBelowPartSize` variants.
