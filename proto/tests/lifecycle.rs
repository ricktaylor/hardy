...
/// the SDK surfaces `on_unregister` to the application, and the handle
/// reports the ending as the disconnection it is — nobody here asked
/// for it.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
...
async fn dropping_the_sink_unregisters() {
    ...
    // Dropping the sink is this side asking to end, so the session ends
    // cleanly rather than reading as a disconnection.
    timeout(handle.join())
        .await
...
async fn a_server_restart_disconnects_the_client() {
    ...
    // The bridge tears its sessions down with clean trailers, so there
    // is no stream failure to report: the handle classifies the ending
    // instead, and a close this side never asked for is a disconnection.
    assert!(
        matches!(
            timeout(handle.join()).await,
            Err(services::Error::Disconnected)
...
// A producer that never yields a segment: a peer that stalled
// mid-transfer, holding a send open with nothing to write.
...
/// A streamed request answered early is answered, however slow its
/// producer: the server rejects the send of a dead session before
/// reading a byte of it, and that status comes back rather than
/// waiting on a producer that never speaks again. The SDK drives the
/// call and the pump feeding it together, so a pump awaited to
/// completion would strand every early response behind the local
/// stream. One test for all three streamed request calls (the
/// application and service `Send`, the CLA `Dispatch`), which share
/// the pump.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
