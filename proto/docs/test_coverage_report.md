...
| `an_abandoned_collection_defers_to_the_next_registration` | APP-06 | In-band cancel ends the call cleanly and commits nothing; the spent stream is `NOT_FOUND` and the next registration collects it whole |
...
| `an_abandoned_forwarding_stays_queued` | CLA-03 | In-band cancel ends the call cleanly with no result; the BPA requeues and the re-announcement completes. `#[ignore]`d: needs the BPA to retry a synchronous `Cla::forward` failure by its kind |
