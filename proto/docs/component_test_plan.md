...
| **CLA-03** | `an_abandoned_forwarding_stays_queued` | An in-band cancel mid-`Forward` is `CANCELLED`, the BPA requeues, and the re-announced forwarding streams the whole bundle and completes | Written, `#[ignore]`d: needs the BPA to retry a synchronous `Cla::forward` failure by its kind ([TODO.md](TODO.md)) |
