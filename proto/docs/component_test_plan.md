...
| :--- | :--- |
| **Functional Area** | gRPC API v1 (wire contract, server surfaces, client SDK) |
| **Module** | `hardy-proto` |
...
This document details the component testing strategy for the v1 gRPC surfaces of `hardy-proto`: the four server surfaces (`ApplicationServiceImpl`, `ServiceServiceImpl`, `ClaServiceImpl`, `RoutingAgentServiceImpl`), the shared subscription and hold-table machinery they ride on, and the `BpaClient` SDK, all defined in the [design document](design.md).

**Scope:**

- **Server surfaces over the real wire:** each surface is mounted on a real TCP listener and driven by its generated tonic client, against a real in-process `hardy_bpa::Bpa`.
...
- **Client SDK end to end:** roundtrips and the SDK-specific commit paths per surface through `BpaClient` against the same surfaces, verifying that a component behind the SDK behaves as a local registration would.
...
The tests live beside the code they pin, as `#[cfg(test)]` modules in `src/server/session.rs`, `src/server/announce.rs` (the shared announce-and-collect table), and `src/server/services/{application,service,cla,routing}.rs`, with the shared harness and fixtures in an inline `#[cfg(test)] pub mod tests` in `mod.rs` cross-imported by path. The ten SDK-driven in-crate tests and the ten cross-crate lifecycle tests are additionally gated on the `client` feature, because they drive the server surfaces through `BpaClient` instead of the generated clients.

## 3. Test Suites

### Suite SES: shared session state (`server/session.rs` and the live-session index in `server/subscribe.rs`)

*Objective: pin the invariants of `Session` and `SessionStream` that every surface relies on, without a network.*

| Test ID | Test function | What it pins | Status |
| :--- | :--- | :--- | :--- |
| **SES-01** | *(pinned at the wire)* | Mint, publish, one-probe resolve; a forged token and a retired token are `UNAUTHENTICATED`; removal is idempotent. The index is `Sessions`' own state and is not separately constructible, so this is pinned end to end instead: `a_forged_token_is_rejected` (per surface) and every teardown test's post-teardown rejection | Absorbed |
...
### Suite REG: shared announce-and-collect table (`server/announce.rs`)

*Objective: pin the announce-and-collect primitive that the delivery and forward rendezvous share, without a network.*

| Test ID | Test function | What it pins | Status |
| :--- | :--- | :--- | :--- |
| **REG-01** | `a_collect_is_single_use` | An announced entry is collectable exactly once: a second collect of the same id misses | Implemented |
| **REG-02** | `a_dropped_announcement_is_withdrawn` | An announcement abandoned by its announcer leaves no entry behind, so nothing can collect it | Implemented |
| **REG-03** | `a_stale_announcement_spares_a_successor` | A superseded announcement withdraws only its own entry, leaving a live successor under the same id collectable | Implemented |
...
| **APP-06** | `an_abandoned_collection_defers_to_the_next_registration` | An in-band cancel mid-collection ends the call cleanly and commits nothing; the spent announcement answers `NOT_FOUND`, and the next registration is re-announced the parked bundle and collects the whole ADU | Implemented |
...
| **APP-17** | `pool_shutdown_survives_a_claimed_unread_receive` | A client that claims a large collection and stops reading, keeping its connection alive, does not wedge pool shutdown: the parked writer abandons its terminal status | Implemented |
...
| **APP-24** | `re_registration_re_announces_many_parked_deliveries` | 48 uncollected deliveries survive an unregister; re-registration completes promptly and every parked bundle is re-announced and collectable | Written, `#[ignore]`d: needs per-delivery concurrency in the BPA ([TODO.md](TODO.md)) |
...
| **APP-27** | `an_rpc_abandoned_during_registration_ends_unregistered` | An rpc abandoned while the BPA is committing the registration still ends unregistered: the workflow runs on a task the client cannot cancel, so the commit completes and is then undone | Implemented |
...
| **SVC-03** | `a_truncated_send_never_commits` | Truncation through the streamed writer is `ABORTED` and nothing is submitted | Implemented |
...
| **SVC-10** | `client_sdk_roundtrip` | A service behind `BpaClient` sends a whole bundle through the streamed writer and collects its own delivery | Implemented (`client` feature) |
...
| **CLA-02** | `dispatch_and_forward_roundtrip` | A dispatched bundle for a destination behind an announced peer is routed back out: the dispatch is answered `ACCEPTANCE_ACCEPTED`, the `Forwarding` carries the peer address, the streamed bundle matches the announced size and carries the payload, and the `sent` result completes the call cleanly | Implemented |
| **CLA-03** | `an_abandoned_forwarding_stays_queued` | An in-band cancel mid-`Forward` ends the call cleanly with no result, the BPA requeues, and the re-announced forwarding streams the whole bundle and completes | Written, `#[ignore]`d: needs the BPA to retry a synchronous `Cla::forward` failure by its kind ([TODO.md](TODO.md)) |
...
| **CLA-05** | `a_truncated_dispatch_never_commits` | Truncated dispatch is `ABORTED` and no forwarding is announced | Written, `#[ignore]`d: needs the BPA to commit a streamed ingress bundle only on its final segment ([TODO.md](TODO.md)) |
| **CLA-06** | `a_cancelled_dispatch_is_discarded` | The wire's in-band `cancel` ends the dispatch `CANCELLED` | Written, `#[ignore]`d: the same final-segment gate ([TODO.md](TODO.md)) |
...
- `cargo test -p hardy-proto --features server` runs the 61 in-crate wire and unit tests that do not need the SDK.
- `cargo test -p hardy-proto --all-features` runs all 81, adding the ten SDK-driven in-crate tests and the ten cross-crate lifecycle tests.
- CI runs the workspace with `--all-features`, so all 81 run on every change.
...
The lifecycle scenarios live in `proto/tests/lifecycle.rs`: ten tests driving the `BpaClient` SDK against a served surface through the crate's public surface only.
...
| **LIF-02** | `bpa_initiated_teardown_reaches_the_client` | Shutting the BPA down unregisters the server's component, ends the wire session, the SDK surfaces `on_unregister`, and the registration handle resolves `Err(Disconnected)`: nobody on this side asked for the ending |
...
| **LIF-05** | `dropping_the_sink_unregisters` | An application that never stores its sink has disconnected by definition: the dropped sink half-closes the session, the server unregisters it, the SDK surfaces `on_unregister`, and the handle resolves `Ok` because the drop was this side asking |
| **LIF-06** | `a_server_restart_disconnects_the_client` | A server teardown (a restart from the client's view) surfaces `on_unregister` and, having clean trailers and no local unregister behind it, resolves the handle `Err(Disconnected)`; the orphaned sink fails rather than blocking, its token dead |
...
| **LIF-08** | `deliveries_collect_concurrently` | Two announced bundles are inside `on_deliver` at once, so a slow collection does not stall the next announcement. Written, `#[ignore]`d: needs per-delivery concurrency in the BPA ([TODO.md](TODO.md)) |
...
| **LIF-10** | `a_dead_session_fails_a_send_from_a_stalled_producer` | A streamed request answered before its request stream is read returns that answer, however slow the local producer: a dead session's send fails as `Disconnected` even though its producer never yields a segment. Pins `client::write_transfer`, shared by all three streamed request calls |
