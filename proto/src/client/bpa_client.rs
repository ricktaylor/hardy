// The client SDK's front door: the connection pool, the `register_*`
// methods, and the handle each registration returns. The per-surface
// session machinery lives in `super::services`.
...
/// ended. The SDK classifies the ending, so the caller never has to
/// reconstruct it: `Ok(())` when this side asked for it (the component
/// unregistered or dropped its sink, or the client's pool shut down),
/// `Err(Disconnected)` when the BPA closed a session nobody here asked
/// to end, and `Err` with the real error, source chain intact, on a lost
/// stream. The SDK never reconnects; the caller decides what a lost
/// session means.
///
...
impl<Id, E> RegistrationHandle<Id, E> {
    ...
    /// Awaits the session to its end, consuming the handle: `Ok(())` for
    /// an ending this side asked for, `Err` for any other; see
    /// [`RegistrationHandle`].
...
/// let handle = bpa.register_application(Service::Ipn(42), app.clone()).await?;
///
...
///
/// // Later, handle how the session ended: an error is any ending this
/// // side did not ask for, disconnection included.
...
pub struct BpaClient {
    // One allocation behind every clone, so a client handed around costs
    // a single refcount bump and every clone shares the one round-robin
    // cursor.
    inner: Arc<Inner>,
}
...
#[derive(Debug)]
struct Inner {
    // Sessions are sharded round-robin across it. A `Channel` clone
    // shares its connection, so a whole session pins to one entry.
...
impl BpaClient {
        ...
        D::Error: Into<Box<dyn core::error::Error + Send + Sync>>,
    {
        Ok(endpoint
            ...
            // Sized to the bandwidth-delay product: the fixed ~64 KiB
            // default caps a transfer at window/RTT, throttling
            // GB-scale bundles on any link with real latency.
            .http2_adaptive_window(true)
            // A whole chunk in as few DATA frames as possible: the
            // ~16 KiB default fragments each one into many.
    ...
    pub fn with_endpoint_pool(
        ...
        // Each `connect_lazy` is an independent connection, where
        // cloning a `Channel` would share one.
    ...
    // The next connection in round-robin order. Cloning a `Channel`
    // shares its connection, so every call a session makes off this
    // clone stays on one HTTP/2 state machine, which the session token
    // and its in-band cancel require.
    ...
    /// does after
    /// [`on_deliver`](services::Application::on_deliver)
    ...
    /// A delivery commits only when the SDK acknowledges it, which it
    /// does after
    /// [`on_deliver`](services::Service::on_deliver)
    /// returns `Ok` for a fully received stream. Returning `Err` parks
    /// the bundle for a later registration, whether or not the stream
    /// was received in full; so does a connection lost before the
