...
async fn deliver(
    ...
    let result = tokio::select! {
        biased;
        result = service.on_deliver(&bundle_id, expiry, delivery.bundle_size, &mut reader) => result,
        _ = cancel.cancelled() => Err(services::Error::StreamCancelled),
    };
    match result {
        // Accepted without receiving to completion: a service bug. No
        // ack goes out (an early ack is a protocol violation), so the
        // bundle stays parked for re-delivery.
        Ok(()) if !reader.is_complete() => {
            warn!(
                "Service accepted delivery {} without receiving it in full; it will be re-delivered",
                delivery.bundle_id
            );
        }
        // The service received the whole ADU; acknowledge to commit the
        ...
        Err(e) => log_declined("Service", &delivery.bundle_id, reader.is_complete(), &e),
    }
}
        ...
        match event {
            ...
            subscribe_response::Event::Delivery(delivery) => {
                ...
                hardy_async::spawn!(deliveries, "service_delivery", async move {
                    deliver(service, cancel, bundle_id, expiry, delivery, reader).await
                })
                .await;
            }
