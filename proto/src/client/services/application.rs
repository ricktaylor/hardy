...
impl ApplicationSessionCtx {
    ...
    async fn deliver(
        ...
        let result = tokio::select! {
            biased;
            result = self.application.on_deliver(&bundle_id, expiry, delivery.ack_requested, delivery.adu_size, &mut reader) => result,
...
impl ApplicationSession {
    ...
    /// session — which therefore cannot exist without having registered.
