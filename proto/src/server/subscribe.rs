...
use tokio::sync::{
    mpsc::{Sender as EventSender, channel},
    oneshot,
};
...
pub trait SubscribeHandler: Send + Sync + Sized + 'static {
    ...
    /// `Session::new(token, cancel, events)`, and registers it with the
    /// BPA. Returns the component and the `Registration` event the
    ...
    fn register(
        ...
        events: EventSender<Result<Self::Event, Status>>,
...
impl<S: SubscribeHandler> Sessions<S> {
    ...
    async fn serve(
        ...
        let (events_tx, events_rx) = channel(S::EVENT_DEPTH);
