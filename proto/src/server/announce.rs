...
impl<Rsp, Req> Announced<'_, Rsp, Req> {
    ...
    /// the streams of its call, or the [`SessionError`] for the two ways
    /// a wait ends without one.
    ///
    ...
    ) -> Result<Collection<Rsp, Req>, SessionError> {
        tokio::select! {
            biased;
            _ = cancelled.cancelled() => Err(SessionError::Closed),
            // A door that took the entry always answers it, so the only
            // way the sender is dropped unanswered is a successor
            // replacing this announcement in the table.
            call = &mut self.call_rx => call.map_err(|_| SessionError::Superseded),
        }
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn a_superseded_announcement_is_not_a_session_ending() {
        let announcements = Table::default();
        let a = ticket(1);

        let mut stale = announcements.announce(&a);
        let _live = announcements.announce(&a);

        // The wait ends on the successor's insert, not on the token, so
        // an announcer whose entry was replaced must not report the
        // session as gone.
        assert!(
            matches!(
                stale.collected(&CancellationToken::new()).await,
                Err(SessionError::Superseded)
            ),
            "a superseded announcement must not read as a closed session"
        );
    }
}
