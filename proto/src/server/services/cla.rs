...
impl SubscribeHandler for GrpcCla {
    ...
    async fn register(
        ...
        let cla = Arc::new(GrpcCla::new(Session::new(token.clone(), cancel, events)));
    ...
    async fn unregister(&self) {
        if let Some(registered) = self.registered.peek() {
...
impl Cla for GrpcCla {
    ...
    async fn forward(
        ...
        // The session dying here leaves the bundle queued in the BPA
        // for a later registration.
        let cancelled = self.session.cancellation();
        let Some(Collection {
            responses_tx,
            requests,
        }) = announced.collected(&cancelled).await
        else {
            return Err(cla::Error::Disconnected);
        };
        ...
        match answer {
            Ok(result) => Ok(result),
            // try_send, best effort: a CLA that abandoned, or whose
            // session has died, may not be reading.
            Err(status) => {
