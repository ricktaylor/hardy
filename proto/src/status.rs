...
server surfaces embed on their error path, the client SDK recovers in
...
The status codes themselves are deliberately not here: only the tag
tables need lockstep. Each side's code mapping is its own one-way
policy, and the two are not inverses. The server picks codes (and
redacts internals) in its `*_status` functions, next to the doors
that answer with them; the client classifies codes it could not
recover a tag from in its `*_error` functions. This module stays
featureless and policy-free so the contract-only build carries it
whole.
*/
...
// and recovery falls back to the code mapping.
...
pub fn embed_service_error(status: Status, e: &services::Error) -> Status {
    let (kind, detail) = match e {
        ...
        // Three variants the BPA never constructs, matched for
        // exhaustiveness alone (see `docs/TODO.md`). An unreachable arm
        // should not mint wire contract, so they travel with no
        // discriminator and the SDK classifies them by code. Delete this
        // arm with the variants.
