...
- `server` — the BPA-side servers: one `ServiceImpl` per surface, serving the wire over any `hardy_bpa::bpa::BpaRegistration` for a host to mount on its own tonic transport.
