...
- **`codec`** -- low-level PDU encode/decode, including transparent
  handling of received bare BPv6/BPv7 bundle frames on shared links.
...
- **`sender` / `receiver`** -- ready-to-use `Sender` and `Receiver` for
  typical CLA implementations. The `Sender` is configured per link with a
  `LinkFraming`: fixed-size frames (padded PDUs, the default) or
  variable-length PDUs, optionally emitting fitting bundles as bare frames
...
```rust
use bytes::Bytes;
use hardy_btpu::receiver::{MaxBundleSize, Receiver, ReceiverEvent};
use hardy_btpu::sender::{PduSize, SendOpts, SendQueueDepth, Sender};
use hardy_btpu::transfer::WindowSize;

// PduSize and WindowSize validate their spec-defined ranges on
// construction (via TryFrom); the defaults are always valid. The spec
// recommends an unpredictable initial transfer number; with the `rand`
// feature, use `Sender::from_rng(..., &mut rng)` instead.
let mut sender = Sender::new(PduSize::default(), WindowSize::default(), SendQueueDepth::default(), read_hw_rng());
sender.enqueue(Bytes::from(bundle_data), SendOpts::default())?;
while sender.has_pending() {
    transmit_over_link(&sender.next_pdu().unwrap());
}

// Receiver side, with the default 1 GiB bundle-size cap.
...
- **`serde`** -- `Serialize`/`Deserialize` for the wire and configuration
  types; `PduSize` and `WindowSize` deserialize from plain integers with
  their range validation applied.
