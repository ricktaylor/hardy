...
- `sender::LinkFraming`: per-link choice between fixed-size frames (every PDU padded to the `PduSize`) and variable-length PDUs (unpadded), the latter optionally emitting a fitting, hint-free bundle as a bare bundle frame. Bare frames share the sender's single pending queue, so they keep arrival order behind queued transfers and count against the `SendQueueDepth` instead of bypassing it.
