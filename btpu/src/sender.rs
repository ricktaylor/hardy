use crate::codec;
use crate::header::{HEADER_SIZE, MAX_CONTENT_LENGTH};
...
///
/// A PDU of this size can be exactly filled by a single message carrying the
/// maximum 20-bit content length.  Any message or padding content the sender
/// derives from a `pdu_size` within this bound is guaranteed encodable;
/// beyond it, segment capacities and padding lengths would overflow the
/// 20-bit length field.
pub const MAX_PDU_SIZE: usize = HEADER_SIZE + MAX_CONTENT_LENGTH;
...
pub struct SenderConfig {
    /// Maximum convergence layer PDU size in bytes (at most [`MAX_PDU_SIZE`]).
    /// Default: 1500.
    pub pdu_size: usize,
...
pub struct Sender {
    ...
    /// Transfer numbers holding a window slot.  [`Self::complete`] and
    /// [`Self::cancel`] only release a slot for numbers present here, so
    /// bogus or duplicate calls cannot free slots that were never allocated.
    active: BTreeSet<u32>,
...
impl Sender {
    ...
    /// # Panics
    ///
    /// Panics if `config.pdu_size` exceeds [`MAX_PDU_SIZE`] or
    /// `config.window_size` is outside
    /// [`MIN_WINDOW_SIZE`](crate::transfer::MIN_WINDOW_SIZE)..=[`MAX_WINDOW_SIZE`](crate::transfer::MAX_WINDOW_SIZE).
    pub fn new(config: SenderConfig, initial_transfer_number: u32) -> Self {
        assert!(
            config.pdu_size <= MAX_PDU_SIZE,
            "pdu_size {} exceeds MAX_PDU_SIZE {MAX_PDU_SIZE}",
            config.pdu_size
        );
        Self {
    ...
    #[cfg(feature = "rand")]
    pub fn from_rng<R: rand_core::Rng>(config: SenderConfig, rng: &mut R) -> Self {
    ...
    pub fn enqueue(&mut self, data: Bytes) -> Result<Option<u32>, Error> {
        ...
        self.active.insert(transfer_number);
    ...
    pub fn cancel(&mut self, transfer_number: u32) {
        if !self.active.remove(&transfer_number) {
            return;
        }
...
mod tests {
    ...
    fn max_pdu_size_accepted() {
        make_sender(MAX_PDU_SIZE);
    }

    #[test]
    #[should_panic(expected = "exceeds MAX_PDU_SIZE")]
    fn oversized_pdu_size_rejected() {
        make_sender(MAX_PDU_SIZE + 1);
    }
    ...
    fn max_pdu_size_bundle_encodes_without_panic() {
        ...
        let mut s = make_sender(MAX_PDU_SIZE);

        // Largest bundle that fits unsegmented: content == MAX_CONTENT_LENGTH.
        s.enqueue(Bytes::from(alloc::vec![0u8; MAX_CONTENT_LENGTH]))
            .unwrap();
        // One byte more: must segment, and every segment must encode.
        s.enqueue(Bytes::from(alloc::vec![0u8; MAX_CONTENT_LENGTH + 1]))
            .unwrap();
    ...
    fn from_rng_seeds_initial_transfer_number() {
        ...
        let t = s.enqueue(Bytes::from(vec![0; 200])).unwrap().unwrap();
    ...
    fn bogus_complete_does_not_free_slot() {
        ...
        for _ in 0..4 {
            s.enqueue(Bytes::from(vec![0; 200])).unwrap().unwrap();
        }
        assert!(s.enqueue(Bytes::from(vec![0; 200])).is_err());
    ...
    fn bogus_cancel_is_noop() {
        ...
        let tn = s.enqueue(Bytes::from(vec![0; 200])).unwrap().unwrap();
