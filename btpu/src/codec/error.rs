...
pub enum Error {
    ...
    /// CBOR, Section 12.1).  Mid-PDU this marks an encapsulated bundle whose
    /// extent cannot be determined without parsing the bundle itself, so per
    /// Section 7.3 decoding stops here and the remainder is left unprocessed.
