...
/// while a framing fault (truncated header, length past the buffer, or a
/// bundle-reserved type byte mid-stream) yields a final [`Err`] and stops
/// iteration permanently — without a boundary the remaining bytes cannot be
/// walked. [`MessageIter::is_exhausted`] distinguishes the two after an
/// error.
///
...
/// has been parsed) marks an encapsulated bundle in its native format
/// (Section 7.3; the ranges are reserved for exactly that distinction,
/// Section 12.1). A bundle carries no BTP-U header to give its extent, and
/// this decoder does not parse bundle formats to find one, so per Section
/// 7.3 it MUST NOT attempt to process the remainder of the PDU. The byte is
/// therefore a terminal framing error ([`Error::ReservedMessageType`]):
/// messages already yielded are unaffected, and the remainder is discarded.
...
impl MessageIter {
    ...
    fn frame_next(&self) -> Result<(MessageHeader, usize)> {
        // A bundle-reserved byte mid-stream marks an encapsulated bundle
        // (Section 7.3; Section 12.1 reserves the ranges for that
        // distinction), which carries no BTP-U header to give its extent.
        // This decoder does not parse bundle formats to find one, so Section
        // 7.3 forbids processing the remainder of the PDU.
...
fn decode_message(
    ...
    // Every message is self-framing (Section 7 TLV header), and Section 7.3
    // requires an unrecognized type to be skipped via the header length
    // field and processing to continue; it is preserved opaquely, its
    // content (hint bytes included) uninterpreted, so a malformed or
    // extension-defined hint chain in an unknown message cannot error the
