...
draft-ietf-dtn-btpu-03 and draft-ietf-dtn-btpu-fec-01; this is the only place
...
- **`tower`**: Implements [`tower::Service<Bytes>`] for [`sender::Sender`]
  ...
  `Sender::complete` / `Sender::cancel`) or the send queue is at its
  configured [`sender::SendQueueDepth`] (woken when `Stream::poll_next`
  drains a PDU); `Stream::poll_next` parks while the
  ...
  `Arc<Mutex<_>>` (not `tower::buffer::Buffer`, which strands the `Stream`
  half and `complete`/`cancel` in its worker task).
*/
