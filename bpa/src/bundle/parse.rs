...
/// applied, the deferred block-1 op-sets drained out of `bib_ops` for the
/// dispatcher's payload drain to re-verify as the payload streams; the §E
/// removals are deferred to the output doors too). On a keyed failure the
/// recoverable bundle rides the error.
#[allow(clippy::result_large_err)]
fn verify_headers(
    headers: &[u8],
