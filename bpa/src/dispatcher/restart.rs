...
mod tests {
    ...
    async fn restart_re_announces_waiting_for_service() {
        ...
        let (bpv7, data) = hardy_bpv7::builder::Builder::new(
            "ipn:0.3.1".parse().unwrap(),
        ...
        let data = Bytes::from(data);
        let storage_name = data_store.save(data.clone()).await.unwrap();
        let extensions = crate::bundle::parse::extract_from_built(&bpv7, &data).unwrap();
        let mut metadata = bundle::BundleMetadata::originated().with_extensions(extensions);
        metadata.storage_name = Some(storage_name);
        let bundle = bundle::Bundle {
            bpv7,
            metadata,
