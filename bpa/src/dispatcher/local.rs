...
impl Dispatcher {
    ...
    pub async fn local_dispatch(
        ...
        // id is unique by construction — `CreationTimestamp::now` issues
        // process-monotonic `(time, sequence)` pairs — so `DuplicateBundle`
        // here can only mean a collision with a pre-restart bundle (a wall
        // clock that stepped backwards across a restart), surfaced to the
        // caller like any other rejection.
        let mut builder =
