...
Open residue: whether `ForwardItem.bundle` stays a full `Bundle` or leans down once the queue tranche makes queues durable (start with `Bundle`, let the queue work argue for less), and the intra-chain classification read question in [Open questions](#open-questions). Refinement (2026-09-01, landed with the C3 engine swap): the resolved adjacency EID rides the **queue-assignment record** — `ForwardPending { peer, queue, next_hop }` — anticipating `ForwardItem.next_hop`. A queue-level constant was considered and rejected: a peer registers one NodeId per EID scheme, so there is no 1:1 peer→EID mapping. The `metadata.next_hop` transient is gone (`Rib::find` takes `&Bundle`; its `Forward` result carries `{ peer, next_hop }` into the peer queue's `send_to`), and the hybrid channel's storage spill can no longer lose the decision: the backends persist it in their status columns, recovery matches on queue *identity* (`BundleStatus::same_queue` — the per-bundle payload is not part of it) and re-delivers each record intact, while the ownership CASes keep full-equality semantics.
