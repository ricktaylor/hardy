...
| **Status** | Living doc — partially implemented (streaming parser, CLA segment delivery, unified ingress pipeline with pre-drain gate + validating `TailReceiver` drain landed; storage / egress / tee'd spool pending) |
...
- The drain is concurrent: a spawned task owns the `TailReceiver` and the spool (fed by a bounded-channel pump over the borrowed CLA stream), racing the gate decisions, with a cancel token aborting the spool on a chain or route drop — §5.7's shape, landed. What remains interim is only the spool's RAM destination above.
