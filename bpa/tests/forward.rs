...
///
/// Every successfully buffered `forward` call is recorded, then answered
/// from the front of `script`, or with `Sent` once the script is exhausted.
struct BufferedCla {
    sink: hardy_async::sync::spin::Once<Box<dyn cla::Sink>>,
    events_tx: flume::Sender<Event>,
...
async fn buffered_cla_receives_whole_bundle() {
    ...
    let (cla, events_rx) = BufferedCla::new(Vec::new());
...
// module so its helpers do not collide with the door and buffer_stream
// tests above; the mock CLAs are the file-level ones.
// ---------------------------------------------------------------------------
mod outcomes {
    use std::sync::Arc;
    ...
    use super::{BufferedCla, Event, Scripted, StreamingCla};
    ...
    impl MetadataStorage for SignalingMem {
        ...
        async fn update_status(&self, bundle_id: &Id, status: &BundleStatus) -> StorageResult<()> {
            let result = self.store.update_status(bundle_id, status).await;
            let _ = self.tx.send(());
            result
        }
    ...
    /// parks in `Waiting` (an unchanged routing decision is not re-run at
    /// pipeline speed), and the next routing event re-offers it. The
    /// successful retry resolves it terminally rather than looping.
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn synchronous_forward_error_parks_bundle_until_routing_event() {
        ...
        // The failure parks the bundle in Waiting. Parked means parked:
        // the status was reached without a second offer in between.
        watcher
            .wait(&id, "the failed bundle to park in Waiting", |st| {
        ...
        // the script is exhausted, so the offer is answered Sent.
        cla.sink()
    ...
    async fn interrupted_streamed_forward_is_redispatched_and_retried() {
        ...
        // The flaky first attempt fails mid-stream without pulling it.
        ...
        assert_eq!(retried, id, "The retry must be the same bundle");
        assert!(matches!(segments.last(), Some(Segment::Final(_))));

        // `Sent` resolves the retry terminally: exactly one retried forward.
        watcher
            .wait(&id, "the retried bundle to be deleted", |st| st.is_none())
            .await;
