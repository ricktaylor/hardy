...
- `cla::MAX_LANE_COUNT` — the public bound on explicit lane declarations, so wire bridges can reject an out-of-range `lane_count` at the registration door with the same limit the BPA clamps to.
...
- **BREAKING:** `ApplicationSink::send` takes the payload as a segment stream — `send(destination, lifetime, options, size_hint, stream: &mut dyn stream::Receiver<Segment>)` — bounded by `max_bundle_size` like the other doors; a caller holding the whole payload passes it directly, since `Bytes` implements `stream::Receiver<Segment>`. The new `size_hint: Option<u64>` parameter lets a genuinely streaming caller declare the payload size up front so the BPA pre-sizes reassembly; it is advisory and clamped by `max_bundle_size`.
- **BREAKING:** `services::Error` (not `#[non_exhaustive]`): `DtnInvalidServiceName`, `NoIpnNodeId`, and `NoDtnNodeId` are replaced by `NodeId(node_ids::Error)`, which carries the node-ID resolution failure whole; new `InvalidSource` variant for a send whose bundle source is not the registration's endpoint.
- **BREAKING:** `stream::concat_stream` takes a third `size_hint: Option<u64>` parameter — the pre-sizing that `buffer_stream` always had, now available to callers with a declared but inexact size; pass `None` for the previous behaviour.
- A `Cla::forward` failure is retried by its kind: an interrupted transfer (`StreamCancelled`, a stream abandoned or truncated mid-forward) re-dispatches over the same route immediately, while any other error (the CLA rejecting the bundle: oversized, bad address) parks the bundle in `Waiting` to retry on the next routing event, bounded by the reaper. Previously every error re-dispatched immediately, so a deterministic rejection from an in-process CLA cycled dispatch→egress→forward at pipeline speed until the bundle's lifetime expired.
...
- A registration poll can no longer capture a bundle parked by its own in-flight delivery and deliver it a second time concurrently: in-flight deliveries are marked in a dispatcher-local set the poll consults before claiming, closing the window between `WaitingForService` (parked mid-announcement) and `WaitingForService` (parked awaiting a registration), which the status value alone cannot distinguish. A delivery completing in the same instant as an unregister is now honoured instead of being re-parked and re-announced as a duplicate.
