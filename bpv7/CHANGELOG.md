...
### Changed
- **BREAKING:** `editor::BlockBuilder::rebuild` is `editor::BlockBuilder::build`. It finishes one block and hands the `Editor` back; it never rebuilt anything, and the old name collided with `Editor::rebuild` / `Editor::rebuild_bundle`, which do rebuild the whole bundle and are unchanged. Signature and behaviour are identical.
...
- The AES key-wrap helpers return a typed error (invalid KEK length vs failed operation) instead of `Result<_, String>`, and RNG failure is the new deliberately detail-free `bpsec::Error::Rng` (was a stringified `Algorithm`).
