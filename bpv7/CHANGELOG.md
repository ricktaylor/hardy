...
### Added
- The parser bounds every pre-payload offset to 256 MiB (new `Error::ExtensionBlocksTooLarge`): every extension block, and the payload block's header, must end within the bound — an implementation limit; the whole pre-payload region must be resident for verification, and real header chains are kilobytes. Rejected from the declared block lengths alone, before any body byte arrives, identically on every node regardless of pointer width. Only the payload body may exceed the bound (`Block::extent`/`payload_range` stay `u64`), so a parsed header extent always converts to `usize`, and a hostile header-chain declaration can no longer invite spooling bytes the parser must reject anyway.
