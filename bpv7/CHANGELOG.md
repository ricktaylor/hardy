...
### Changed
- `creation_timestamp::CreationTimestamp::now` issues strictly monotonic `(time, sequence)` pairs per process, from one atomic: bundles created in the same millisecond take ascending sequence numbers, and a wall clock that steps backwards never re-issues an earlier pair — so ids built from `now()` are unique by construction and callers need no collision checks. Previously the sequence number was derived from the clock's nanoseconds alone, so two reads landing on the same instant (coarse clocks, concurrent issuers) could collide. Uniqueness across process restarts still rests on the wall clock moving forward (RFC 9171 §4.2.7).
