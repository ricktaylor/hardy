...
struct BundleSide<'a> {
    bundle: &'a Bundle,
    data: &'a [u8],
    /// Blocks grouped by type code: (BlockType, sorted block numbers).
...
fn compare_known_extension(
    ...
    match bt {
        BlockType::PreviousNode => compare_decoded::<Eid>(a_body, b_body, tag, diffs),
        BlockType::BundleAge => compare_decoded::<BundleAge>(a_body, b_body, tag, diffs),
        BlockType::HopCount => compare_decoded::<HopInfo>(a_body, b_body, tag, diffs),
...
fn resolve_targets(
    ...
    index_map: &BTreeMap<u64, (BlockType, usize)>,
