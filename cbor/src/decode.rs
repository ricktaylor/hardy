...
pub enum Error {
    ...
    #[error("Unparsed items in sequence")]
    UnparsedItems,
}
...
impl<'a, const D: usize> Sequence<'a, D> {
    ...
    pub fn try_parse_map<T, F>(&mut self, f: F) -> Result<Option<(T, usize)>, anyhow::Error>
    where
    ...
    pub fn parse_map<T, F>(&mut self, f: F) -> Result<(T, usize), anyhow::Error>
    where
        F: FnOnce(&mut Map, usize, &[u64]) -> Result<T, anyhow::Error>,
    {
...
pub fn parse_map<T, F>(data: &[u8], f: F) -> Result<(T, usize), anyhow::Error>
where
