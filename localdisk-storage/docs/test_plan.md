...
| **7.1** | Store/retrieve bundles | [`PLAN-STORE-01`](../../tests/storage/docs/test_plan.md) Suite D (BLOB-01..05) |
...
| **LD-05** | **Write failure handling** | `storage.rs` | _Unix only._<br>1. Make store directory read-only.<br>2. Probe that writes are actually blocked (a privileged process bypasses permission bits; the assertion is skipped in that case).<br>3. Call `save(data)`. | Graceful error returned, not panic. |
