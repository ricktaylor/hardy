...
async fn test_alloc1_matching() {
    ...
    let Parsed {
        ...
        ..
    } = parse::parse(Bytes::copy_from_slice(&new_data)).unwrap();
...
async fn test_mixed_source_only_rewrite() {
    ...
    assert!(
        matches!(parsed_bundle.primary.destination, Eid::Dtn { .. }),
    ...
    assert_eq!(
        parsed_bundle
            .blocks
            .get(&1)
            .and_then(|block| block.payload(&parsed_data)),
...
async fn test_mixed_dest_only_rewrite() {
    ...
    assert!(
        matches!(parsed_bundle.primary.id.source, Eid::Dtn { .. }),
