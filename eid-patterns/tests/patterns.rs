...
fn invalid_syntax_rejected() {
    assert!(matches!(
        "ipn:1-1".parse::<EidPattern>(),
        Err(Error::ParseError(_))
    ));
    assert!(matches!(
        "http://*".parse::<EidPattern>(),
        Err(Error::ParseError(_))
    ));
    assert!(matches!(
        "".parse::<EidPattern>(),
...
fn numeric_boundaries() {
    ...
    assert!(pattern_matches("ipn:0.4294967295.1", "ipn:0.4294967295.1"));
    // u32 overflow must reject, not wrap.
    assert!(matches!(
        "ipn:0.4294967296.1".parse::<EidPattern>(),
        Err(Error::ParseError(_))
    ));
    ...
    assert!(matches!(
        "99999999999999999999999:**".parse::<EidPattern>(),
        Err(Error::ParseError(_))
    ));
}
