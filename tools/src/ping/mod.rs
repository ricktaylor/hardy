...
/// reflected payload byte-for-byte against what was sent. That comparison
/// covers only the payload: every other part of the response bundle is freshly
/// generated by the echo service rather than reflected, so there is nothing
/// else for the client to verify it against (see draft-taylor-dtn-echo-service).
...
pub struct Command {
    ...
    /// dtn7-rs's dtnecho2) reflect bundles without a primary-block CRC. The
    /// reflected payload is still verified by the byte-for-byte comparison.
    #[arg(long)]
    lax_rfc9171: bool,
