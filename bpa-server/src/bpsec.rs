...
impl PatternKeySource {
    ...
    /// Loads the JWKS file named in `config` and builds a `PatternKeySource`.
    ///
...
pub fn watch_keys(tasks: &TaskPool, config: BPSecConfig, provider: Arc<PatternKeyProvider>) {
    ...
    hardy_async::spawn!(tasks, "key_file_watcher", async move {
        watcher::watch(&keys_file, watch_mode, cancel, move || {
            ...
            async move {
                ...
                match PatternKeySource::load(&config) {
                    Ok(source) => {
                        provider.set(source);
...
mod load_tests {
    ...
    fn config_with(keys_path: &Path, bindings: Vec<KeyBindingConfig>) -> BPSecConfig {
        BPSecConfig {
    ...
    fn load_succeeds_with_valid_config() {
        ...
        let source = PatternKeySource::load(&config).unwrap();
    ...
    #[test]
    fn missing_key_file() {
        let config = BPSecConfig {
            keys_file: PathBuf::from("/nonexistent/keys.jwks"),
            watch: WatchConfig::None,
            bindings: vec![],
        };
        assert!(PatternKeySource::load(&config).is_err());
    }
