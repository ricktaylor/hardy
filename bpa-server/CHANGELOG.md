...
### Changed
- **BREAKING**: the `ipn-legacy-filter` cargo feature is removed (it was in the default set): the IPN legacy 2-element re-encode is now a built-in of the `hardy-bpa` per-hop rewrite stage and is always available. The `ipn-legacy-nodes` config key is unchanged in shape (a list of EID patterns) and now works in every build rather than only feature-enabled ones.
- `max-bundle-size` is a 64-bit value (tracking `BpaBuilder::max_bundle_size` becoming `NonZeroU64`); on 32-bit targets a value beyond the address space is accepted and saturates in the BPA.
