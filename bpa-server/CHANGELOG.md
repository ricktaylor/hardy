...
- HTTP/2 transport tuning for the gRPC listener via an optional `grpc.http2` block: `adaptive-window` (default on) auto-sizes the flow-control windows to the connection's bandwidth-delay product so a single large transfer is no longer capped at `window / round-trip-time` by the fixed default window; `initial-stream-window-size`/`initial-connection-window-size` pin fixed windows instead; `max-concurrent-streams` and `max-frame-size` bound per-connection memory and cut per-frame overhead. Absent keys defer to the server defaults. The client SDK's default `Endpoint` (`BpaClient::new`) likewise enables the adaptive window.
