...
async fn test_response_swaps_endpoints() {
    ...
    let Parsed {
        ...
        bundle: parsed,
        ..
    } = parse::parse(hardy_bpa::Bytes::copy_from_slice(response)).unwrap();
    assert_eq!(parsed.primary.id.source, ENDPOINT.parse().unwrap());
    assert_eq!(parsed.primary.destination, "ipn:1.1".parse().unwrap());
    assert_eq!(
        parsed.primary.lifetime,
        core::time::Duration::from_secs(7200)
    );
    assert_eq!(
        parsed.blocks.get(&1).and_then(|block| block.payload(&data)),
        Some(b"echo-me".as_slice())
    );
    assert_eq!(parsed.primary.flags, Flags::default());
}
...
async fn test_response_mirrors_report_flags_and_report_to() {
    ...
    assert_eq!(parsed.primary.report_to, report_to);
}
