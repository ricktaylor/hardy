...
if [ "$USE_DOCKER" = true ]; then
    ...
    if ! docker image inspect "$CFS_IMAGE" &>/dev/null; then
        ...
        docker build -t "$CFS_IMAGE" "$SCRIPT_DIR/docker"
    else
...
if [ "$USE_DOCKER" = true ]; then
    ...
    log_info "Waiting for cFS to initialize (15s)..."
    sleep 15
    ...
    log_info "cFS container running"
else
...
log_step "Hardy pinging cFS echo service at ipn:$CFS_NODE_NUM.42 via STCP..."
echo ""

PING_OUTPUT=$(timeout 30s "$BP_BIN" ping "ipn:$CFS_NODE_NUM.42" \
    --cla "$MTCP_CLA_BIN" \
...
# Dump cFS logs for debugging
log_info "cFS container logs:"
docker logs "$CFS_CONTAINER" 2>&1 | grep -i 'BPNODE\|stcpsock\|STCP\|contact\|application\|Failed\|Error\|listen\|accept\|connect\|Adding\|Starting\|Setup' | grep -v 'Child Task' | tail -30
echo ""

# Stop cFS
log_info "Stopping cFS..."
if [ "$USE_DOCKER" = true ]; then
    docker stop "$CFS_CONTAINER" 2>/dev/null || true
    docker rm -f "$CFS_CONTAINER" 2>/dev/null || true
    CFS_CONTAINER=""
fi
