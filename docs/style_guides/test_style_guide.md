...
- **No hard-coded cryptographic values, even in tests.** Never write literal keys, IVs, or salts, including in `#[cfg(test)]` modules. When the value is immaterial to the assertion (a round trip with whatever key was used, a rejection that fires before the key is read), generate it through the crate's own randomness helper where one exists. The one exception is externally pinned vectors — spec appendices, conformance/PICS suites, interop captures — where the expected bytes were produced with that key; those stay verbatim.
