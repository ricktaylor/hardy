...
| `initial-stream-window-size` | Integer up to `2147483647` (2^31 - 1, RFC 9113 §6.9.1) | *(transport default)* | Fixed initial per-stream receive window. Ignored while `adaptive-window` is on. Out-of-range values are a parse error. |
| `initial-connection-window-size` | Integer up to `2147483647` (2^31 - 1, RFC 9113 §6.9.1) | *(transport default)* | Fixed initial whole-connection receive window. Ignored while `adaptive-window` is on. Out-of-range values are a parse error. |
| `max-concurrent-streams` | Non-negative integer | *(transport default, ~200)* | Maximum concurrent HTTP/2 streams a peer may open. Bounds per-connection memory (window &times; streams), and doubles as a throughput knob: each transfer is its own RPC, so this caps a connection's concurrent in-flight transfers. Raise it, or pool connections client-side, to push more transfers in parallel. |
