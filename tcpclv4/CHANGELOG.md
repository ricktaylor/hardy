...
- **BREAKING**: the TLS debug flag `tls.debug.accept-self-signed` is replaced by `tls.insecure`, and the `tls.debug` section is removed. `tls.private-key-file` is renamed to `tls.key-file` (the old name remains accepted as an alias). Exactly one of `tls.ca-certs` or `tls.insecure` must be configured; setting both, or neither, is a startup error.
