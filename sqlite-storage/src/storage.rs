...
// The poll_pending page shapes, hoisted so the plan pin test below EXPLAINs
...
// The ForwardPending poll leaves status_param3 unconstrained (each row's
// param3 carries its own resolved adjacency, selected back to the caller);
// the status index orders received_at before status_param3 precisely so
// this shape is still served in index order.
const POLL_FORWARD_PENDING_SQL: &str = "SELECT bundle, status_param3 FROM bundles
...
impl MetadataStorage for SqliteStorage {
    ...
    async fn poll_pending(
        ...
        let bundles = self
            .read(move |conn| {
                if forward_pending {
                    conn.prepare_cached(POLL_FORWARD_PENDING_SQL)?
                        .query_map(
                            (status_code, status_param1, status_param2, limit as isize),
                    ...
                    conn.prepare_cached(POLL_PENDING_SQL)?
                        .query_map(
                            ...
                            |row| Ok((row.get::<_, Vec<u8>>(0)?, None)),
                        )?
                        .collect::<Result<Vec<(Vec<u8>, Option<String>)>, _>>()
                        .map_err(Into::into)
                }
...
mod tests {
    ...
    use super::{
        POLL_FORWARD_PENDING_SQL, POLL_PENDING_SQL, SqliteStorage, from_status, to_status,
    };
    ...
    fn poll_pending_plan_has_no_temp_btree() {
        ...
        // The ForwardPending shape leaves status_param3 unconstrained; the
        // index orders received_at ahead of status_param3 so this plan must
        // also sort in-index.
        let plan: Vec<String> = conn
            .prepare(&format!("EXPLAIN QUERY PLAN {POLL_FORWARD_PENDING_SQL}"))
            .unwrap()
            .query_map(
                rusqlite::params![2i64, Some(60i64), Some(3i64), 16isize],
                |row| row.get::<_, String>(3),
            )
            .unwrap()
            .collect::<Result<_, _>>()
        ...
        assert!(
            !plan.iter().any(|step| step.contains("TEMP B-TREE")),
            "forward-pending poll plan sorts out of index: {plan:?}"
        );
    }
