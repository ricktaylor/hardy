...
### Changed
- **BREAKING**: a CLA session closed by the BPA (its shutdown or restart) exits nonzero, so a `Restart=on-failure` supervisor restarts the CLA once the BPA is back; previously a BPA-initiated close exited 0 and the CLA stayed down. SIGINT/SIGTERM still exit 0, and a lost connection still exits nonzero.
- The configured transfer MRU is declared to the BPA at registration, so the effective size cap reported back (and advertised to peers in `SESS_INIT`) folds the BPA's own limit with the MRU.
